...
const splitInput = input.map((string) => ({
	firstNumber: string.split(": ")[0].split(" ")[0].split("-")[0],
	secondNumber: string.split(": ")[0].split(" ")[0].split("-")[1],
	letter: string.split(": ")[0].split(" ")[1],
	password: string.split(": ")[1]
}));

const validPasswordCount = splitInput.reduce((count, currentInput) => {
	const letterCountInPassword = (currentInput.password.match(new RegExp(currentInput.letter, 'gi')) || []).length;
	if (letterCountInPassword >= currentInput.firstNumber && letterCountInPassword <= currentInput.secondNumber) return count + 1
	return count
}, 0)

console.log(validPasswordCount)

const validPasswordCount2 = splitInput.reduce((count, currentInput) => {
	const firstChar = currentInput.password.charAt(currentInput.firstNumber - 1);
	const secondChar = currentInput.password.charAt(currentInput.secondNumber - 1);

	if ((firstChar === currentInput.letter) !== (secondChar === currentInput.letter)) return count + 1
	return count
}, 0)
